...
private:
    ...
    double lvcVRC[4][3];
    int lscVRC[4][2];       // 布局图中的视图参考坐标系（相对于屏幕坐标系）
    double lwcPRP[2][3];    // 布局图中的投影参考点（相对于世界坐标系）
    double lvcPRP[2][3];
    int lscPRP[2][2];       // 布局图中的投影参考点（相对于屏幕坐标系）
...
public:
    ...
    // 投影变换
    void Project(double MS[][3], int MO[][2], int nRow, double MT[4][4]);
    // 屏幕变换
    void ToScreen(int MSp[][2], int nRow);
    // 矩阵乘法
